...
model.fc = nn.Linear(model.fc.in_features, 4)   # simplest possible head
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(),
                   os.path.join(ROOT_DIR, "outputs/models/baseline_resnet50.pth"))
        print("✓ Best model saved.")

pd.DataFrame(metrics).to_csv(
    os.path.join(ROOT_DIR, "outputs/logs/baseline_resnet50_metrics.csv"), index=False)
